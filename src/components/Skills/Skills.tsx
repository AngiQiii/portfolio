    ...
    <div id="skills" className=" w-full h-auto mb-10">
      ...
      <div className="w-full h-auto flex items-center justify-evenly mb-10">
        {skillsData.map((skill) => (
          <div className="bg-primary rounded-full w-32 h-32 flex flex-col items-center justify-center shadow-sm shadow-slate-700">
            <div>{skill.image}</div>
            <p>{skill.name}</p>
          </div>
        ))}
      ...
      <div className="flex items-center justify-evenly text-center">
        {certificates.map((certificate) => (
          <a
            className="bg-primary mb-2 w-60 py-3 px-5 shadow-sm shadow-slate-700"
            target="_black"
